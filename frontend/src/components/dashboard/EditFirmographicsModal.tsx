...
import {
  ...
  EditDialogContent as DialogContent,
  EditDialogFooter as DialogFooter,
  EditDialogHeader as DialogHeader,
  EditDialogTitle as DialogTitle,
} from "../ui/dialog";
...
interface EditFirmographicsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: FirmographicRow[]) => void;
  initialRows: FirmographicRow[];
}

const colorOptions = ["yellow", "blue", "red", "gray", "green", "purple"];
...
export default function EditFirmographicsModal({ isOpen, onClose, onSave, initialRows }: EditFirmographicsModalProps) {
  ...
  const handleAddValue = (rowId: string) => {
    ...
    setRows((prev) =>
      prev.map((row) => {
        if (row.id === rowId) {
          const existingColors = row.values.map((v) => v.color);
          const availableColors = colorOptions.filter((color) => !existingColors.includes(color));
    ...
    setRows((prev) =>
      prev.map((row) => {
        if (row.id === rowId) {
          return {
            ...row,
            values: row.values.filter((_, index) => index !== valueIndex),
          };
        }
        return row;
      })
